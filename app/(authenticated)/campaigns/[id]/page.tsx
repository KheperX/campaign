...
export default async function CampaignDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-gray-900">{campaign.campaign_name}</h1>
      <div className="mt-4">
        <h2 className="text-xl font-semibold text-gray-900">Campaign Details</h2>
        <div className="mt-2 grid grid-cols-2 gap-4">
          <div>
            <p className="text-sm font-medium text-gray-500">Start Date</p>
            <p className="mt-1 text-sm text-gray-900">{new Date(campaign.campaign_start_date).toLocaleDateString()}</p>
          </div>
          <div>
            <p className="text-sm font-medium text-gray-500">End Date</p>
            <p className="mt-1 text-sm text-gray-900">{new Date(campaign.campaign_end_date).toLocaleDateString()}</p>
          </div>
          <div>
            <p className="text-sm font-medium text-gray-500">Type</p>
            <p className="mt-1 text-sm text-gray-900">{campaign.campaign_type}</p>
          </div>
          <div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {tickets.map((ticket) => (
                      <tr key={ticket.ticket_id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{ticket.status}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{new Date(ticket.createdAt).toLocaleString()}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{ticket.usedAt ? new Date(ticket.usedAt).toLocaleString() : '-'}</div>
                        </td>
                      </tr>
                    ))}
                  </tbody>
